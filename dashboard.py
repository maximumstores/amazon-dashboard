...
def show_orders(t=None):
    ...
    # Sessions в S&T report перекриваються між child-ами одного parent-а.
    # Правильно: MAX(sessions) per (parent, date) → потім SUM по parent-ах.
    try:
        with engine.connect() as conn:
            parent_traffic = pd.read_sql(text("""
                WITH parent_daily AS (
                    SELECT
                        ...
                        MAX(NULLIF(page_views,'')::numeric)   AS pv,
                        MAX(NULLIF(sessions,'')::numeric)      AS sess,
                        SUM(NULLIF(units_ordered,'')::numeric) AS units
                    FROM spapi.sales_traffic
                    WHERE report_date::date BETWEEN :d1 AND :d2
                      AND child_asin != '' AND child_asin IS NOT NULL
                      AND granularity = 'SKU'
                    GROUP BY parent_asin, report_date::date
                )
                SELECT
                    SUM(pv)    AS total_pv,
                    SUM(sess)  AS total_sess,
                    SUM(units) AS total_units_traffic
                FROM parent_daily
            """), conn, params={"d1": d1, "d2": d2}).iloc[0]
...
<div style="background:linear-gradient(135deg,#1a2b1e,#0d1f12);border:1px solid #2d4a30;
  ...
  <div style="flex:1;min-width:200px;display:flex;gap:8px;flex-wrap:wrap;align-items:center">
    ...
    # Для daily chart: sessions per parent MAX, потім SUM across parents
    # (sessions між child-ами одного parent перекриваються)
    daily_parent_sess = df.groupby(['date', 'parent_asin']).agg(
    ...
    daily = daily_orders.merge(daily_parent_sess, on='date', how='left')
    ...
    if parent_agg.empty:
        ...
        html_parts.append("""
<style>
.ph-wrapper { max-height:700px; overflow-y:auto; border-radius:8px;
              border:1px solid #2d2d4a; }
.ph-table { width:100%; border-collapse:collapse; font-size:13px;
            background:#0e1117; color:#fff; }
.ph-table thead { position:sticky; top:0; z-index:10; }
...
def show_custom_quality():
    ...
    if asin_input.strip():
        asins = [a.strip().upper() for a in re.split(r'[,\s\n]+', asin_input) if a.strip()]
        if asins:
    ...
    if sel_size:
        df_f = df_f[df_f['size'].astype(str).str.strip().isin(sel_size)]
    if sel_brand:
        df_f = df_f[df_f['brand'].astype(str).str.strip().isin(sel_brand)]
    if sel_mp != "Всі":
        df_f = df_f[df_f['marketplace'].astype(str).str.contains(sel_mp.replace("Amazon.", ""), case=False, na=False)]
    if sel_fc == "FBA (AMAZON_NA)":
        df_f = df_f[df_f['fulfillment_channel'].astype(str).str.contains("AMAZON", case=False, na=False)]
    ...
    st.markdown("---")
    st.markdown("#### 🧬 Список ASIN для аналізу")

    unique_asins = sorted(df_f['asin1'].dropna().astype(str).str.strip().unique().tolist())
    unique_asins = [a for a in unique_asins if a]

    c1, c2, c3 = st.columns(3)
    c1.metric("Унікальних ASIN", f"{len(unique_asins):,}")
    c2.metric("Рядків (SKU)",   f"{len(df_f):,}")
    c3.metric("Активних ASIN",
              f"{df_f[df_f['status'].astype(str).str.lower()=='active']['asin1'].nunique():,}")

    fmt = st.radio("Формат виводу:",
                   ["Через кому", "З нового рядка", "JSON-масив"],
                   horizontal=True, key="cq_asin_fmt")

    if fmt == "Через кому":
        asin_text = ", ".join(unique_asins)
    ...
    with cdl1:
        st.download_button("📥 ASIN .txt",
                           ...
                           "asins.txt", "text/plain", key="dl_cq_asins_txt")
    with cdl2:
        st.download_button("📥 ASIN .csv",
...
NAV_I18N = {
    ...
    "💸 Reimbursement":                {"UA": "💸 Reimbursement",      "EN": "💸 Reimbursement",    "RU": "💸 Reimbursement"},
...
elif report_choice == "💸 Reimbursement":            show_reimbursement(t)
